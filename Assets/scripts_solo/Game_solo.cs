...
public class Game_solo : MonoBehaviour
{
    ...
    public int turn;
    public Player_solo Me;
    public Player_solo You;
    public Unrev_solo unrev;
    public string selected_card;
    public string situation;
    public GameObject maincanvas;
    public List<int> AIchoiceList = new List<int>(){0,1,2,3,4,5,6,7};
    public List<int> AIacceptList;
    ...
    public static List<List<string>> playersCards;
    public static List<string> boxcards;
    ...
    public void turn_pass()
    {
        ...
        if (turn%2 == 1 && turn!= 17)
        {
            ...
            temp1.GetComponent<Card_solo>().Front_Back = true;
            temp1.GetComponent<Card_solo>().Cardtype = RevealCard;
            ...
            temp2.GetComponent<Card_solo>().Front_Back = true;
            temp2.GetComponent<Card_solo>().Cardtype = RevealCard;
    ...
    public void showdown()
    {
        ...
        // temp.GetComponent<Card_solo>().Cardtype = selected_card;
        // temp.GetComponent<RectTransform>().sizeDelta = new Vector2(189,264);
        // temp.name = "ShowdownCard";
        if (IsMyTurn)
        {
            StartCoroutine(AIacting());
        }
    }

    public void AIchoiceSet()
    {

        for (int i = 0; i < AIchoiceList.Count; i++)
        {
            int randomIndex = UnityEngine.Random.Range(i, AIchoiceList.Count); // i�??�� list.Count ?��?��?��?�� 무작?�� ?��?��?�� ?��?��
            int temp = AIchoiceList[i];
            AIchoiceList[i] = AIchoiceList[randomIndex];
            AIchoiceList[randomIndex] = temp;
        }
        Debug.Log($"AIchoice: {string.Join(", ", AIchoiceList)}");
    }

    public void AIacceptSet()
    {
        for (int i = 0; i < 8; i++)
        {
            int randomIndex = UnityEngine.Random.Range(0,2); // i�??�� list.Count ?��?��?��?�� 무작?�� ?��?��?�� ?��?��
            AIacceptList.Add(randomIndex);
        }
        Debug.Log($"AIaccept: {string.Join(", ", AIacceptList)}");
    }

    public void AIchoice()
    {
        selected_card = playersCards[1][AIchoiceList[(turn/2)-1]];
        You.transform.GetChild(AIchoiceList[(turn/2)-1]).transform.GetChild(2).GetComponent<CardCanvas_solo>().Card_select();
    }

    public void AIaccept()
    {
        if(AIacceptList[(turn-1)/2] == 1)
        {
            accept();
            ap.SetActive(true);
        }
    ...
    public void accept()
    {
        ...
        if (IsMyTurn)
        {
            StartCoroutine(RotateObject(GameObject.Find("Me").transform.Find("Card_" + GameObject.Find("Game").GetComponent<Game_solo>().selected_card + "_canvas").gameObject));
            GameObject.Find("Me").transform.Find("Card_" + GameObject.Find("Game").GetComponent<Game_solo>().selected_card + "_canvas").transform.GetChild(0).GetComponent<Card_solo>().Front_Back = true;
            StartCoroutine(destroyshowdown("Me"));
            ap.SetActive(true);
        }
        else
        {
            StartCoroutine(RotateObject(GameObject.Find("You").transform.Find("Card_" + GameObject.Find("Game").GetComponent<Game_solo>().selected_card + "_canvas").gameObject));
            GameObject.Find("You").transform.Find("Card_" + GameObject.Find("Game").GetComponent<Game_solo>().selected_card + "_canvas").transform.GetChild(0).GetComponent<Card_solo>().Front_Back = true;
    ...
    IEnumerator destroyshowdown(string a)
    {
        ...
        GameObject.Find(a).transform.Find("Card_" + GameObject.Find("Game").GetComponent<Game_solo>().selected_card + "_canvas").GetComponent<CardCanvas_solo>().BackOrigin();
        situation = "card_selecting";
    }
    ...
    IEnumerator RotateObject2(GameObject a)
    {
        ...
        while (timeElapsed < duration)
        {
            ...
            GameObject.Find("ShowdownCard").GetComponent<Card_solo>().Front_Back = true;
        a.transform.rotation = Quaternion.Euler(0, -90,0);
        startRotation = a.transform.rotation;
        targetRotation = Quaternion.Euler(0, 0, 0);
        timeElapsed = 0;
    ...
    public void UpdateNIckname()
    {
        ...
        Me.playername = UserInfo.Data.nickname;
    }
