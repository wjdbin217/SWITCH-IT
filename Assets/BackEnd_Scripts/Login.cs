...
public class Login : LoginBase
{
    ...
    public Animator animatorPW;
    public Animator animatorbtnLogin;
    ...
    private void ResponseToLogin(string Id, string Pw)
    {
        ...
        Backend.BMember.CustomLogin(Id, Pw, callback=>
        {
            ...
            if (callback.IsSuccess())
            {
                ...
                animatorID.SetTrigger("IDl");
                animatorPW.SetTrigger("PWr");
                animatorbtnLogin.SetTrigger("btnLoginl");
                animator.SetTrigger("iconu");
                Invoke("GoToNextScene", 3.0f);
            }
            //로그인 실패
            else
            {
                //로그인에 실패했을 때는 다시 로그인을 해야 하기 때문에 "로그인 버튼" 상호작용 활성화
                ...
                switch( int.Parse(callback.GetStatusCode()))
                {
                    ...
                    case 403:   //유저 or 디바이스 차단
                        message = callback.GetMessage().Contains("user") ? "차단당한 유저 입니다." : "차단당한 디바이스입니다.";
                        break;
                    case 410:   //탈퇴 진행중
